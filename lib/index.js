	...
	Sequelize.prototype.defineAll = function(definitions, defineOptions) {
		...
		function prepOne(modelName, definition)
		{
			...
			if (!key && defineOptions.primaryKey) {
				...
				var keyField = {type: defineOptions.primaryKeyType, allowNull: false, primaryKey: true, autoIncrement: true, _autoGenerated: true};
		...
		function defineOne(modelName, definition)
		{
			...
			_.defaultNoDef(options, 'autoAssociate', defineOptions.autoAssociate);
			_.defaultNoDef(options, 'labels', defineOptions.labels);
		...
		function defineRelationships()
		{
			...
			_.forEach(relationshipsMany, function(options) {
				...
				if (options.through) {
					...
					// make through table
					var fields = {};
					fields[modelName1 + Utils.uppercaseFirst(key1)] = {type: definitions[modelName1].fields[key1].type, allowNull: false, _autoGenerated: true};
					fields[modelName2 + Utils.uppercaseFirst(key2)] = {type: definitions[modelName2].fields[key2].type, allowNull: false, _autoGenerated: true};
