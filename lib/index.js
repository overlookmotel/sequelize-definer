...
module.exports = function(Sequelize) {
	...
	Sequelize.prototype.defineAll = function(definitions, defineOptions) {
		...
		{
			// inherit options from defineOptions (except defineOptions.fields)
		...
		function defineOne(modelName, definition)
		{
			...
			_.forIn(fields, function(params, fieldName) {
				...
				if (reference !== undefined && !reference) {
					...
					params.type = referenceField ? referenceField.type : options.primaryKeyType;
		...
		function defineRelationships()
		{
			...
			_.forEach(relationshipsMany, function(options) {
				...
				if (options.through) {
					...
					if (options.labels) {
						fields[foreignKey1].label = utils.camelToHuman(modelName1);
						fields[foreignKey2].label = utils.camelToHuman(modelName2);
					}
					
					var modelName = modelName1 + (options.camel ? Utils.uppercaseFirst(modelName2) : modelName2);
					var tableName = options.freezeTableName ? modelName : (Utils.pluralize(modelName1) + Utils.pluralize(options.camel ? Utils.uppercaseFirst(modelName2) : modelName2));
