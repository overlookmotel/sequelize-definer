...
module.exports = function(definitions, defineOptions) {
	...
	function prepOne(modelName, definition)
	{
		...
		if (!key) {
			...
			var shouldAutoIncrement = options.primaryKeyType.toString().match(/(INTEGER|BIGINT)/);
			...
			if (options.primaryKeyType.toString() === 'UUID') {
				keyField.defaultValue = Sequelize.UUIDV4;
			}
