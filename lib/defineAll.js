	...
	function defineRelationships() {
		...
		_.forEach(relationshipsMany, (options) => {
			...
			if (modelName1 === modelName2 && (!options.as || !options.asReverse)) {
				throw new Sequelize.DefinerError(`'as' and 'asReverse' must be provided for many-to-many self-joins in model '${modelName1}'`);
			}
			...
			const key1 = definitions[modelName1].primaryKey,
				...
				foreignKey2 = patches.underscoredIf(
					`${modelName1 === modelName2 ? Utils.singularize(options.as) : modelName2}${utils.uppercaseFirst(key2)}`,
					options.underscored
				);
			...
			if (options.through) {
				...
				const tableName = makeTableName(modelName, options);
