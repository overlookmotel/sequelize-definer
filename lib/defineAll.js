...
module.exports = function(definitions, defineOptions) {
	...
	function prepOne(modelName, definition)
	{
		...
		var primaryKeyDef = {model: modelName, field: key};
		...
		// get table name
		options.tableName = makeTableName(modelName, options);
	...
	function defineOne(modelName, definition)
	{
		...
		if (options.labels) {
			['createdAt', 'updatedAt', 'deletedAt'].forEach(function(fieldName) {
				...
				if (fieldName && model.rawAttributes[fieldName]) model.rawAttributes[fieldName].label = utils.humanize(fieldName);
			});
		}
	...
	function defineRelationships()
	{
		...
		_.forEach(relationshipsMany, function(options) {
			...
			if (options.through) {
				...
				var tableName = makeTableName(modelName, options);
			...
			if (options.primaryKeyThrough) delete through.rawAttributes[options.through.primaryKeyAttribute]._autoGenerated;
	...
	function makeTableName(modelName, options) {
		if (options.tableName) return options.tableName;
		if (options.freezeTableName) return modelName;
