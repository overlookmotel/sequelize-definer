...
module.exports = function(definitions, defineOptions) {
	...
	defineOptions = Object.assign({ // eslint-disable-line prefer-object-spread
		primaryKey: 'id',
	...
	function defineOne(modelName, definition) {
		...
		_.forIn(fields, (params, fieldName) => {
			...
			if (reference !== undefined && !reference) {
				...
				if (reference) {
					...
					let as;
					if (fieldName.endsWith(utils.uppercaseFirst(params.referenceKey))) {
						as = fieldName.slice(0, -params.referenceKey.length);
						if (!params.as && as !== reference) params.as = as;
						label = params.as || as;
					} else if (fieldName.endsWith(`_${params.referenceKey}`)) {
		...
		if (definition.manyToMany) {
			_.forIn(definition.manyToMany, (optsArr, modelName2) => {
				if (!Array.isArray(optsArr)) optsArr = [optsArr];
	...
	function defineRelationships() {
		...
		for (const options of relationshipsOne) {
		...
		for (const options of relationshipsMany) {
