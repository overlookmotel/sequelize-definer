...
module.exports = function(definitions, defineOptions) {
	...
	function prepOne(modelName, definition)
	{
		...
		if (!key) {
			if (!options.primaryKey.name) throw new Sequelize.SequelizeDefinerError("Model '" + modelName + "' has no primary key defined");

			key = utils.functionValue(options.primaryKey.name, modelName, definition, definitions);

			var shouldAutoIncrement = ['INTEGER', 'BIGINT'].indexOf(options.primaryKey.type._typeName) != -1;
			var keyField = {
				type: options.primaryKey.type,
				allowNull: false,
				primaryKey: true,
				autoIncrement: shouldAutoIncrement,
				defaultValue: options.primaryKey.defaultValue,
				_autoGenerated: true
			};

			if (options.primaryKey.type == Sequelize.UUID && !options.primaryKey.defaultValue) keyField.defaultValue = Sequelize.UUIDV4;
	...
	function defineOne(modelName, definition)
	{
		...
		['primaryKey', 'primaryKeyFirst', 'autoAssociate', 'labels', 'onDelete', 'onUpdate'].forEach(function(optionName) {
